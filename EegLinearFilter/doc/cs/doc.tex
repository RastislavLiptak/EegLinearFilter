...
Dalším důležitým ukazatelem je, že stabilita optimalizovaného sekvenčního i paralelního výpočtu je vysoká (viz obr. \ref{fig:cpu_par_auto_vec}). Rozptyl naměřených časů je minimální a pohybuje se v řádu desítek milisekund (odchylka pod 2 \%).
...
\toprule
\textbf{Implementace} & \textbf{$T_s$ [s]} & \textbf{$T_p$ [s]} & \textbf{Zrychlení ($S$)} & \textbf{Karp-Flatt ($e$)} & \textbf{Gustafson ($S_{scaled}$)} \\
\midrule
\texttt{NAIVE}       & 42,24 & 5,92 & 7,14 & 0,017 (1,7 \%) & 7,88 \\
\texttt{NO\_VEC}     & 17,11 & 2,87 & 5,96 & 0,049 (4,9 \%) & 7,66 \\
\texttt{AUTO\_VEC}   & 10,17 & 1,70 & 5,98 & 0,048 (4,8 \%) & 7,66 \\
\texttt{MANUAL\_VEC} & 10,27 & 1,69 & 6,07 & 0,045 (4,5 \%) & 7,68 \\
\bottomrule
\end{tabular}%
}
\end{table}

Nejvyšší hodnoty zrychlení ($S=7,13$) a nejnižší sériovou režii ($e=1,7\,\%$) vykazuje naivní implementace. Kvůli silné datové závislosti a nevyužívání registrů je výpočet limitován rychlostí samotného výpočtu, nikoliv propustností paměti. Jednotlivá vlákna tak nezahlcují sběrnici a nebrzdí se navzájem, což se blíží ideálnímu lineárnímu škálování podle Amdahlova zákona.

Naopak u optimalizovaných variant (\texttt{NO\_VEC}, \texttt{AUTO\_VEC} a \texttt{MANUAL\_VEC}) dochází k poklesu zrychlení na úroveň $S \approx 6,0$ a nárůstu sériové režie na cca $4,5\text{--}4,9\,\%$. Skutečnost, že tento limit postihuje i nevektorizovanou verzi (\texttt{NO\_VEC}), dokazuje, že již samotná algoritmická optimalizace zrychlila výpočet natolik, že 8 jader naráží na limit propustnosti paměťové sběrnice. Dalším důkazem tohoto tvrzení je vysoká hodnota Gustafsonova zákona ($S_{scaled} > 7,6$), která dokládá, že samotný algoritmus je navržen efektivně a úzkým hrdlem jsou výhradně sdílené hardwarové prostředky.
%
%
%
